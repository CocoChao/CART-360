...
//      float scaledZOld = mapf(analogRead(zPin), 0, 675, -3, 3);
     ...
     int loopsAmount = 250/amount; // so that the final amount of loops * delay time always equals 750
      for (int i = 0; i < amount; i++) { // if there is one or two directions
        ...
        switch (currentAction) { // depending on the direction
          case 0: // thumb - left
              if (scaledXNew >= (scaledXOld+0.2)) {
               correct = true;
             }
            break;
          case 1: // index - up
              if (scaledZNew >= (scaledZOld+0.2)) {
               correct = true;
             }
            break;
          case 2: // middle - forward
              if (scaledYNew >= (scaledYOld+0.2)) {
               correct = true;
             }
            break;
          case 3: // ring - down
  ...
  // Print out scaled X,Y,Z accelerometer readings
     ...
     // giving a response through the C or F motor pin if the action is correct or incorrect
        if (win) {
          ...
          delay(750);
          stopDance();
        } else if (!win) { // if the player lost, F pin
